...
class AD5933RealtimeBode:
    ...
    def update_plot(self, frame):
        ...
        if self.freq_data:
            ...
            imp_smooth = self.smooth_series(
                imp_vals,
                window=7
            )

            phase_smooth = self.smooth_series(
                phase_vals,
                window=7
            )

            z_real_smooth = self.smooth_series(
                z_real_vals,
                window=7
            )

            z_imag_smooth = self.smooth_series(
                z_imag_vals,
                window=7
            )
            ...
            self.line_phase_smooth.set_data(
                freqs,
                phase_smooth
            )
            ...
            self.line_nyquist_smooth.set_data(
                z_real_smooth,
                z_imag_smooth
            )
